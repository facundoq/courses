...
void gaussian_blur(const unsigned char* const inputChannel,
	...
	for (int filter_r = -half_width; filter_r <= half_width;
			++filter_r) {
		for (int filter_c = -half_width; filter_c <= half_width;
				++filter_c) {
			...
			float filter_value=filter[(filter_r+half_width)*filterWidth+(filter_c+half_width)];

			f += image_value * filter_value;
		}
	}

	outputChannel[thread_1D_pos] = f;
...
void allocateMemoryAndCopyToGPU(const size_t numRowsImage,
		const size_t numColsImage, const float* const h_filter,
	...
	checkCudaErrors(
			cudaMemcpy(d_filter, h_filter, numBytes, cudaMemcpyHostToDevice));
}
...
void your_gaussian_blur(const uchar4 * const h_inputImageRGBA,
	...
	int blockRows = numRows / BS;
	if (blockRows * BS < numRows) {
		blockRows++;
	}
	int blockCols = numCols / BS;
	if (blockCols * BS < numCols) {
		blockCols++;
	}
