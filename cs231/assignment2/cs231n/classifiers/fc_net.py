...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    x1, fc_cache1 = affine_forward(X, W1, b1)
    o1, relu_cache1 = relu_forward(x1)

    o2, fc_cache2 = affine_forward(x1, W2, b2)
    ...
    loss+=reg*0.5*(np.sum(W2*W2)+np.sum(W1*W1))

    dx2, dW2, db2 = affine_backward(dout, fc_cache2)

    dx1 = relu_backward(dx2, relu_cache1)
    dx, dW1, db1 = affine_backward(dx1, fc_cache1)
