...
n=length(genotypeFactor.val);
% Replace the zeros in genotypeFactor.val with the correct values.
assignments=IndexToAssignment(1:n,genotypeFactor.card);
for i=1:n
    son_genotype=assignments(i,1);
    father_genotype=assignments(i,2);
    mother_genotype=assignments(i,3);
    son_alleles=sort(genotypesToAlleles(son_genotype,:));
    
    father_alleles=genotypesToAlleles(father_genotype,:);
    mother_alleles=genotypesToAlleles(mother_genotype,:);
    parents_alleles=cartesian(father_alleles,mother_alleles);
    
    for k=1:size(parents_alleles,1)
        genotypeFactor.val(i)=genotypeFactor.val(i)+0.25*all(son_alleles==sort(parents_alleles(k,:)));
    end
