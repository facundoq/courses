...
%
% Input:
%   alleleWeights: Cell array of weights of m elements (m is the number of genes)
%   , where each entry is an 1 x n array
...
n=length(phenotypeFactor.val);
assignments=IndexToAssignment(1:n,phenotypeFactor.card);

%alleleWeights = {[3, -3], [0.9, -0.8]}; 
genes=length(geneCopyVarOneList);
for i=1:2:n
    alleles_per_gene=assignments(i,2:end);
    alleles_per_gene1=alleles_per_gene(1:genes);
    alleles_per_gene2=alleles_per_gene(genes+1:end);
    weighted_sum=0;
    for j=1:genes
        gene_allele_weights=alleleWeights{j};
        weighted_sum=weighted_sum+gene_allele_weights(alleles_per_gene1(j));
        weighted_sum=weighted_sum+gene_allele_weights(alleles_per_gene2(j));
    end
    phenotypeFactor.val(i)=sigmoid(weighted_sum);
