...
for i=1:numPeople
    ...
    if (all(pedigree.parents(i,:)==[0,0]))
        %genotype for people without parents
        factorList(geno1)= childCopyGivenFreqsFactor(alleleFreqs, geno1);
        factorList(geno2)= childCopyGivenFreqsFactor(alleleFreqs, geno2);
    else
        %genotype for people with parents
        parent1_geno1=pedigree.parents(i,1);
        parent1_geno2=parent1_geno1+numPeople;
        parent2_geno1=pedigree.parents(i,2);
        parent2_geno2=parent2_geno1+numPeople;
        factorList(geno1)= childCopyGivenParentalsFactor(numAlleles, geno1, parent1_geno1,parent1_geno2);
        factorList(geno2)= childCopyGivenParentalsFactor(numAlleles, geno2, parent2_geno1,parent2_geno2);
    end
    %phenotypes
    factorList(pheno)=phenotypeGivenCopiesFactor(alphaList,numAlleles, geno1, geno2,pheno);
end
