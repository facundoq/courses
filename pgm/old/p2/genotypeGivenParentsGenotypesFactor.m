...
% alleles -- need to add number of alleles at the end to account for homozy

genotype_count = nchoosek(numAlleles, 2) + numAlleles;
genotypeFactor = struct('var', [genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo], 'card', repmat(genotype_count, 1, 3), 'val', []);
...
for i=1:genotype_count
    for j=1:genotype_count
        p1_alleles = genotypesToAlleles(i, :);
        p2_alleles = genotypesToAlleles(j, :);
        pairs = [p1_alleles(1) p2_alleles(1); p1_alleles(1) p2_alleles(2); p1_alleles(2) p2_alleles(1); p1_alleles(2) p2_alleles(2)];        
        for k=1:size(pairs,1);
            child_genotype_index = allelesToGenotypes(pairs(k,1), pairs(k,2));
            conditional_child_genotype_index = AssignmentToIndex([child_genotype_index i j], genotypeFactor.card);
            genotypeFactor.val(conditional_child_genotype_index) = genotypeFactor.val(conditional_child_genotype_index) + 0.25;
        end;
    end;    
end;
