...
genotypeFactor = struct('var', genotypeVar, 'card', genotype_count, 'val', zeros(1, genotype_count));
...
for i=1:numAlleles
    for j=1:numAlleles
       prob = alleleFreqs(i) * alleleFreqs(j);
       k = allelesToGenotypes(i, j);
       genotypeFactor.val(k) = genotypeFactor.val(k) + prob;
    end;
end;
