...
phenotypeFactor.card = [2 repmat(numAlleles, 1, length(phenotypeFactor.var) - 1)];
...
for i=1:length(phenotypeFactor.val)
    ...
    pheno = assignment(1);    
    geno = assignment(2:end);  
    net = 0;
    for g=1:length(geno)
      gene_index = mod(g-1, 2) + 1;
      net = net + alleleWeights{gene_index}(geno(g));  
    end;
    y = computeSigmoid(net);
    if pheno == 2
        y = 1 - y;
    end;
    phenotypeFactor.val(i) = y;
end;
