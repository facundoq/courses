...
factor = struct('var', [i j], 'card', [K K], 'val', ones(prod([K K]), 1));

% Your code here:
indexes = AssignmentToIndex(repmat((1:K)', 1, 2), [K K]);
factor.val(indexes) = ImageSimilarity(images(i).img, images(j).img);
